...
local newBaseButton = require(
                          "devgit:source/libraries/UI/components/buttons/baseButton.lua")

return function(props)
    -- Defaults
    props.containerBackgroundColour = colour.hex("#f21b1b")
    ...
    self.container:on("mouseLeftDown",
                      function() self.state.dispatch {type = "activate"} end)
    self.container:on("mouseLeftUp",
                      function() self.state.dispatch {type = "deactivate"} end)
    ...
    self.state.subscribe(function(state)
        if state.enabled then
            ...
            if state.active then
                props.containerBackgroundColour = colour.hex("#bf1d1d")
                props.borderAlpha = 1
                props.borderInset = 5
            elseif state.hovering then
                props.containerBackgroundColour = colour.hex("#ed2f2f")
