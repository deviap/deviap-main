...
return function(props)
	...
	self.container:child("box"):on(
					"mouseExit", function()
						self.states.dispatch({type = "unhover"})
						self.states.dispatch({type = "deactivate"})
					end)
	self.container:child("box"):on(
					"mouseLeftUp", function() self.states.dispatch({type = "deactivate"}) end)
	self.container:child("box"):on(
					"mouseLeftDown", function() self.states.dispatch({type = "activate"}) end)
	...
	self.render = function(state)
		...
		if state.active then
			...
			core.tween:begin(
							self.container:child("box"), 0.1, {strokeColour = colour.hex("#212121")}, "outCirc")
			self.selected = not self.selected
		else
			if state.hovering then
				core.tween:begin(
								self.container:child("box"), 0.1, {strokeColour = colour.hex("#03A9F4")}, "outCirc")
			else
				core.tween:begin(
								self.container:child("box"), 0.1, {strokeColour = colour.hex("#212121")}, "outCirc")
			end
