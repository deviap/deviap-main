...
local createBaseComponent = require(
...
return function(props)
    ...
    self.container:child("box"):on("mouseEnter", function()
        self.states.dispatch({type = "hover"})
    end)
    self.container:child("box"):on("mouseExit", function()
        self.states.dispatch({type = "unhover"})
        self.states.dispatch({type = "deactivate"})
    end)
    self.container:child("box"):on("mouseLeftUp", function()
        self.states.dispatch({type = "deactivate"})
    end)
    self.container:child("box"):on("mouseLeftDown", function()
    ...
    self.render = function(state)
        ...
        if state.enabled then
            ...
            self.container:child("box"):child("boxIcon").backgroundColour =
                colour.hex("#212121")
        else
        ...
        if state.active then
            ...
            core.tween:begin(self.container:child("box"), 0.1,
                             {strokeColour = colour.hex("#212121")}, "outCirc")
