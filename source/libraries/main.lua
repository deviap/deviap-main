...
local stringReplacementForPeriod = ("%source/libraries/"):gsub("%%",
                                                               rootDirectory)
local errorMessageIfModuleFailsToLoad =
    "You either misnamed the library or you forgot './' like a functional programmer."
local regex = "^%."

local errorIfModuleFailsToLoad = function(msg) error(msg, 4) end
...
return function(libraryName)
    return requireModule(globalStringSubstition(
                             librariesRelativePath[libraryName], regex,
                             stringReplacementForPeriod))
end
