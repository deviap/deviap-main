...
core.input:on("keyUp", function(key) if key == "KEY_ESCAPE" then menu.show() end end)

if core.input.hasScreenKeyboard then
	local settingsButton = core.construct(
                       					"guiFrame", {
						parent = core.engine.coreInterface,
	...
	settingsButton:on("mouseLeftDown", function() menu.show() end)
end
