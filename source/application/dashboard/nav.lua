...
controller.sidebar = core.construct(
                     				"guiFrame", {
...
breakpointer:bind(
				controller.sidebar, "xs", {size = guiCoord(1, 0, 0, 150), position = guiCoord(0, 0, 1, -50)})

breakpointer:bind(
				controller.sidebar, "sm", {size = guiCoord(0, 50, 1, 0), position = guiCoord(0, 0, 0, 0)})

controller.activeBall = core.construct(
                        				"guiFrame", {
...
controller.container = core.construct(
                       				"guiFrame", {parent = core.interface, backgroundAlpha = 0, clip = true})

breakpointer:bind(
				controller.container, "xs", {size = guiCoord(1, 0, 1, 0), position = guiCoord(0, 0, 0, 0)})

breakpointer:bind(
				controller.container, "sm", {size = guiCoord(1, -50, 1, 0), position = guiCoord(0, 50, 0, 0)})
...
controller.register = function(module)
	...
	btn:on(
					"mouseLeftDown", function()
						...
						core.tween:begin(
										controller.activeBall, 0.3,
										{position = guiCoord(1, -4, 0, btn.position.offset.y + 20 - 4)}, "inOutQuad")
						page.visible = true
					end)
