...
local container = core.construct(
                  				"guiFrame", {
...
local tween;
tween = core.tween:begin(
        				pattern, 5, {imageTopLeft = vector2(-120, 120)}, "linear", function()
					tween:reset()
					tween:resume()
				end)
...
local colours = {
	colourMap.purple, colourMap.lightBlue, colourMap.blue, colourMap.green, colourMap.terraCotta
}
...
if not core.dev.localDevGitEnabled then
	...
	local infoLabel = core.construct(
                  					"guiTextBox", {
						parent = container,
						size = guiCoord(0, 370, 0, 100),
	...
	local progressBarContainer = core.construct(
                             					"guiFrame", {
						parent = Container,
						size = guiCoord(0, 300, 0, 30),
						position = guiCoord(0.5, -150, 0.64, 10),
	...
	core.engine:on("updateMessage", function(message) infoLabel.text = message end)

	core.engine:on(
					"updateProgress",
					function(str) core.tween:begin(progressBarFrame, 0.2, {size = guiCoord(i / 100, 0, 1, 0)}) end)
end
